...
module c061618g2_core #(
    ...
    // Direct, atomic structural assignment mapping straight to the output pins.
    // If rst_n falls to 0, (~rst_n) becomes 1, cleanly driving the active-low channels high!
    assign core_out = {
        1'b0,                           // unused_p3_b7
        1'b1,                           // FLG_n
        (s4_filtered    | ~rst_n),      // s4_n
        (io_filtered    | ~rst_n),      // io_n
        (ci_filtered    | ~rst_n),      // ci_n
        (os_filtered    | ~rst_n),      // os_n
        (basic_filtered | ~rst_n),      // basic_n
        (s5_filtered    | ~rst_n)       // s5_n
    };
